...
def graph_search(problem, fringe):
    ...
    visitados = 0
    generados = 1 #The graph is never empty so the first generate node is the inicial node
    closed = {}
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            resultado = f"Generados: {generados}\nVisitados: {visitados}\nCosto total: {node.path_cost}\n"
            print(resultado)
            return node
...
def breadth_first_graph_search(problem):
    ...
    bfs_time = f"\nTiempo bfs: {time.time()}"
    print(bfs_time)
    return graph_search(problem, FIFOQueue())  # FIFOQueue -> fringe

def depth_first_graph_search(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    dfs_time = f"\nTiempo dfs: {time.time()}"
    print(dfs_time)
    return graph_search(problem, Stack())

def branch_bound_search(problem):
    bnb_time = f"\nTiempo branch and bound: {time.time()}"
    print(bnb_time)
    return graph_search(problem, order_stack())

def branch_bound_search_with_heuristic(problem):
    bnb_heuristica_time = f"\nTiempo branch and bound with heuristic: {time.time()}"
    print(bnb_heuristica_time)
    return graph_search(problem, heuristic_stack(problem))
