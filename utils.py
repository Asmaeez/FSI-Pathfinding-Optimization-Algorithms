...
class order_stack(Queue):
    """Fifo queue that select the first node with the least cost"""
...
class heuristic_stack(Queue):
    """Fifo queue. Add the cost of the node and the cost to the solucion"""
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A = sorted(self.A, key = lambda i: i.path_cost + self.problem.h(i) , reverse = False)
